...
import { VIP5_BOOKS, VIP5_SET } from './commerce-vip5-catalog.js';
...
/** 5. sınıf VIP 5’li eğitim seti (tek vitrin ürünü + doğru store_kind). */
export async function seedVip5Set(opts = {}) {
  return seedYankiSingleProduct(VIP5_SET, opts);
}

/** 5. sınıf VIP branş + set. */
export async function seedVip5Catalog({ actorSub, prices = {}, contact_phone } = {}) {
  const { vendor, created } = await ensureYankiVendor({ actorSub, contact_phone });
  const books = VIP5_BOOKS.map((b) => {
    const override = prices[b.isbn] || prices[b.slug] || {};
    return {
      ...b,
      price_kurus: Number(override.price_kurus) || 0,
  ...
  const upserted = await bulkUpsertBooks({ books, actorSub, vendorId: vendor.id, approveIfPriced: true });
  const setOut = await seedYankiSingleProduct(VIP5_SET, {
    actorSub,
  ...
  return {
    ...
    set: setOut.book,
    deployMarker: 'vip5-kitap-seed-2026-09-07',
  };
}
