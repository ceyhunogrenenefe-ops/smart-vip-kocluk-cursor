...
/** Belge gönderimi base64 ile ~16MB'a kadar; diğer uçlar küçük JSON */
app.use((req, res, next) => {
  const isDocumentSend = /\/send-document\/?$/i.test(String(req.path || ''));
  express.json({ limit: isDocumentSend ? '22mb' : '1mb' })(req, res, next);
});
...
app.use((err, req, res, next) => {
  if (err?.type === 'entity.too.large' || err?.status === 413 || err?.name === 'PayloadTooLargeError') {
    return res.status(413).json({
      ...
      hint: 'Belge veya istek gövdesi çok büyük. PDF sıkıştırın veya gateway WHATSAPP_JSON_LIMIT_MB artırın.',
    });
  }
  return next(err);
});
