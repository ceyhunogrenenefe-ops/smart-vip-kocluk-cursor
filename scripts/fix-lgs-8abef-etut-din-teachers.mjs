...
async function ensureTeachersOnClass(token, cls) {
  const existingTeachers = Array.isArray(cls.teacher_ids) ? cls.teacher_ids.map(String) : [];
  const existingStudents = Array.isArray(cls.student_ids) ? cls.student_ids.map(String) : [];
  const need = [DOGAN, BUSRA].filter((id) => !existingTeachers.includes(id));
  if (!need.length) return { added: [] };
  if (DRY_RUN) return { added: need, dry: true };
  const teacherIds = [...new Set([...existingTeachers, DOGAN, BUSRA])];
  const { status, json } = await api(
    'POST',
    `/api/class-live-lessons?op=update-class-members`,
    token,
    {
      class_id: cls.id,
...
async function main() {
  ...
  for (const classKey of ['8A', '8B', '8E', '8F']) {
    ...
    console.log(`\n=== ${classKey} ${cls.name} ===`);
    try {
      const linked = await ensureTeachersOnClass(token, cls);
      if (linked.added?.length) {
        console.log(
          `  class_teachers +${linked.added.map((id) => (id === DOGAN ? 'DOĞAN' : 'Büşra')).join(',')}${linked.dry ? ' (dry)' : ''}`
        );
      }
    } catch (e) {
      console.error('  ensure_teachers', e.message || e);
      summary.push({ classKey, error: 'ensure_teachers_failed', detail: String(e.message || e) });
      continue;
    }
    ...
    for (const slot of slots) {
      ...
      console.log(
        `  slot d${slot.day_of_week} ${String(slot.start_time).slice(0, 5)} ${slot.subject}: ${slot.teacher_name} → ${tid === DOGAN ? 'DOĞAN' : 'Büşra'}`
      );
      if (DRY_RUN) {
        slotsOk += 1;
        continue;
    ...
    const sessions = (sessJson.data || []).filter(
      (s) => String(s.status) !== 'cancelled' && targetTeacher(s.subject)
    );
    let sessOk = 0;
    let sessFail = 0;
    for (const sess of sessions) {
      ...
      console.log(
        `  sess ${sess.lesson_date} ${String(sess.start_time).slice(0, 5)} ${sess.subject}: ${sess.teacher_name || sess.teacher_id} → ${tid === DOGAN ? 'DOĞAN' : 'Büşra'}`
      );
      if (DRY_RUN) {
        sessOk += 1;
        continue;
