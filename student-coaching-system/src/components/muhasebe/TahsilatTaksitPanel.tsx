...
export type TahsilatStats = {
  overdueCount: number;
  overdueSums: Record<string, number>;
  dueWeekCount: number;
  dueWeekSums: Record<string, number>;
  thisMonthCount: number;
  thisMonthSums: Record<string, number>;
  totalOpenSums: Record<string, number>;
  openRowCount: number;
};

type Props = {
  compactHeader?: boolean;
  onStatsChange?: (stats: TahsilatStats) => void;
};
...
export function TahsilatTaksitPanel({ compactHeader = false, onStatsChange }: Props) {
  ...
  const stats = useMemo((): TahsilatStats => {
    ...
    <div className="space-y-6">
      {!compactHeader ? (
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <h2 className="text-lg font-bold text-slate-900 dark:text-white">Tahsilat & taksit</h2>
            <p className="text-sm text-slate-600 dark:text-slate-400 mt-1 max-w-xl">
              Veli sözleşmelerindeki taksitleri takip edin; ödeme alındığında satırı işaretleyin.
            </p>
          </div>
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <div className="rounded-2xl border border-amber-200 bg-gradient-to-br from-amber-50 to-white p-4 dark:from-amber-950/30 dark:to-slate-900 dark:border-amber-900/50">
          <p className="text-xs font-semibold text-amber-900 dark:text-amber-200 flex items-center gap-1">
            <CalendarDays className="w-4 h-4" /> Bu hafta / geciken (ödenmemiş)
          </p>
        ...
        <div className="rounded-2xl border border-sky-200 bg-gradient-to-br from-sky-50 to-white p-4 dark:from-sky-950/25 dark:to-slate-900 dark:border-sky-900/40">
          <p className="text-xs font-semibold text-sky-900 dark:text-sky-200">Bu ay vadesi (ödenmemiş)</p>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-4 dark:bg-slate-900 dark:border-slate-700">
          <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">Toplam açık borç</p>
          <p className="text-2xl font-bold text-slate-900 dark:text-white mt-1">{stats.openRowCount} satır</p>
      ...
      <div className="flex flex-wrap items-end gap-3 rounded-2xl border border-slate-200 bg-white p-4 dark:bg-slate-900 dark:border-slate-700">
        ...
        <div className="flex-1 min-w-[160px]">
          <label className="text-xs text-slate-500 dark:text-slate-400">Ara</label>
          <input
      ...
      {loading && !rows.length ? (
        ...
        <div className="rounded-2xl border border-dashed border-slate-300 p-8 text-center text-slate-500 dark:border-slate-600">
          ...
          Taksit satırı yok veya filtreye uyan kayıt yok.
        </div>
      ) : (
...
export default TahsilatTaksitPanel;
