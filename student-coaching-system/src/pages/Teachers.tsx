...
function toSystemUser(u: TeacherUser, fallbackInstitutionId?: string | null): SystemUser {
  const roles = Array.isArray(u.roles)
    ? (u.roles.filter(Boolean) as SystemUser['roles'])
    : undefined;
  const institutionId =
    String(u.institutionId || u.institution_id || fallbackInstitutionId || '').trim() || undefined;
  return {
    id: u.id,
...
export default function Teachers() {
  ...
  const selectedInstId = String(activeInstitutionId || institution?.id || '').trim();
  ...
  const loadTeachers = useCallback(async () => {
    ...
    try {
      ...
      if (effectiveUser?.role === 'super_admin' && selectedInstId) {
        qs.set('institution_id', selectedInstId);
      }
      const path = qs.toString() ? `/api/users?${qs}` : '/api/users';
  ...
  }, [effectiveUser?.role, selectedInstId]);
  ...
  const scopedRows = useMemo(() => {
    if (effectiveUser?.role !== 'super_admin' || !selectedInstId) return rows;
    return rows.filter((u) => {
      const id = String(u.institution_id || u.institutionId || '').trim();
      // institution_id yoksa seçili kurum varsay (eski kayıtlar)
      return !id || id === selectedInstId;
    });
  }, [rows, effectiveUser?.role, selectedInstId]);
  ...
  const handleLoginAs = async (u: TeacherUser) => {
    const target = toSystemUser(u, selectedInstId);
    ...
    if (
      ...
      target.institutionId !== selectedInstId
    ) {
      alert('Bu öğretmen seçili kuruma ait değil. Üstten doğru kurumu seçin.');
      return;
    }
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500">
            Toplam {scopedRows.length} öğretmen
            {effectiveUser?.role === 'super_admin' && institution?.name
              ? ` · ${institution.name}`
              : ''}
          </p>
        </div>
